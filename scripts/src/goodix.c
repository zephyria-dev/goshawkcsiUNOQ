...
static void goodix_poll(struct input_dev *input)
{
...
retry_read_config:
	...
	if (ts->client->irq) {
		...
		dev_info(&ts->client->dev, "No IRQ, using polling mode (%d ms)\n",
			 GOODIX_POLL_INTERVAL_MS);
		input_set_drvdata(ts->input_dev, ts);
		error = input_setup_polling(ts->input_dev, goodix_poll);
		if (error) {
			dev_err(&ts->client->dev, "failed to set up polling: %d\n", error);
			return error;
		}
		input_set_poll_interval(ts->input_dev, GOODIX_POLL_INTERVAL_MS);
		input_set_min_poll_interval(ts->input_dev, 10);
		input_set_max_poll_interval(ts->input_dev, 200);
	}
